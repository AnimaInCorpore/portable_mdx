...
const PRECISION_BITS = 10;
const PRECISION = 1 << PRECISION_BITS;
const SIZEALPHATBL_BITS = 10;
const SIZEALPHATBL = 1 << SIZEALPHATBL_BITS;
const SIZESINTBL_BITS = 10;
const SIZESINTBL = 1 << SIZESINTBL_BITS;
const MAXSINVAL = 1 << (SIZESINTBL_BITS + 2);
const ALPHAZERO = SIZEALPHATBL * 3;

const STEPTBL_O2 = Int32Array.from([
...
const DT1TBL_ORG = Int32Array.from([
...
const XRTBL = Object.freeze([
  { and: 4095, add: 8 }, { and: 2047, add: 5 }, { and: 2047, add: 6 }, { and: 2047, add: 7 }, { and: 2047, add: 8 }, { and: 1023, add: 5 }, { and: 1023, add: 6 }, { and: 1023, add: 7 },
  { and: 1023, add: 8 }, { and: 511, add: 5 }, { and: 511, add: 6 }, { and: 511, add: 7 }, { and: 511, add: 8 }, { and: 255, add: 5 }, { and: 255, add: 6 }, { and: 255, add: 7 },
  { and: 255, add: 8 }, { and: 127, add: 5 }, { and: 127, add: 6 }, { and: 127, add: 7 }, { and: 127, add: 8 }, { and: 63, add: 5 }, { and: 63, add: 6 }, { and: 63, add: 7 },
  { and: 63, add: 8 }, { and: 31, add: 5 }, { and: 31, add: 6 }, { and: 31, add: 7 }, { and: 31, add: 8 }, { and: 15, add: 5 }, { and: 15, add: 6 }, { and: 15, add: 7 },
  { and: 15, add: 8 }, { and: 7, add: 5 }, { and: 7, add: 6 }, { and: 7, add: 7 }, { and: 7, add: 8 }, { and: 3, add: 5 }, { and: 3, add: 6 }, { and: 3, add: 7 },
  { and: 3, add: 8 }, { and: 1, add: 5 }, { and: 1, add: 6 }, { and: 1, add: 7 }, { and: 1, add: 8 }, { and: 0, add: 5 }, { and: 0, add: 6 }, { and: 0, add: 7 },
  { and: 0, add: 8 }, { and: 0, add: 10 }, { and: 0, add: 12 }, { and: 0, add: 14 }, { and: 0, add: 16 }, { and: 0, add: 20 }, { and: 0, add: 24 }, { and: 0, add: 28 },
  { and: 0, add: 32 }, { and: 0, add: 40 }, { and: 0, add: 48 }, { and: 0, add: 56 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 },
  { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 },
  { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 },
  { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 },
  { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 }, { and: 0, add: 64 },
]);

const DT2TBL = Int32Array.from([0, 384, 500, 608]);
...
function fillStaticWaveTables(contextImpl) {
  const sinTbl = contextImpl.m_SINTBL;
  const alphaTbl = contextImpl.m_ALPHATBL;
  const noiseAlphaTbl = contextImpl.m_NOISEALPHATBL;

  for (let i = 0; i < SIZESINTBL; i++) {
    sinTbl[i] = Math.trunc(Math.sin((2.0 * PI * i) / SIZESINTBL) * MAXSINVAL + 0.5);
  }

  alphaTbl.fill(0);
  for (let i = 17; i <= SIZEALPHATBL; i++) {
    alphaTbl[ALPHAZERO + i] = Math.floor(
      Math.pow(2.0, -((SIZEALPHATBL - i) * (128.0 / 8.0)) / SIZEALPHATBL) * PRECISION,
    );
  }

  noiseAlphaTbl.fill(0);
  for (let i = 17; i <= SIZEALPHATBL; i++) {
    noiseAlphaTbl[ALPHAZERO + i] = Math.floor((i / SIZEALPHATBL) * 0.25 * PRECISION);
  }
}

function fillRateDependentTables(contextImpl) {
  const d1lTbl = contextImpl.m_D1LTBL;
  const stepTbl = contextImpl.m_STEPTBL;
  const dt1Tbl = contextImpl.m_DT1TBL;

  const samprate = Math.max(1, toInt(contextImpl.m_Samprate, 22050));
  const opmRate = Math.max(1, toInt(contextImpl.m_OpmRate, 62500));
  ...
  for (let oct = 0; oct <= 10; oct++) {
    for (let notekf = 0; notekf < 12 * 64; notekf++) {
      const step = (oct >= 3)
        ? (STEPTBL_O2[notekf] << (oct - 3))
        : (STEPTBL_O2[notekf] >> (3 - oct));
      stepTbl[oct * 12 * 64 + notekf] = Math.trunc((step * 64 * opmRate) / samprate);
    }
  }

  for (let i = 0; i < DT1TBL_ORG.length; i++) {
    dt1Tbl[i] = Math.trunc((DT1TBL_ORG[i] * 64 * opmRate) / samprate);
  }
}

function initializeX68SoundTables(contextImpl) {
  fillStaticWaveTables(contextImpl);
  fillRateDependentTables(contextImpl);
}
...
export {
  ...
  XRTBL,
  DT2TBL,
  fillRateDependentTables,
  initializeX68SoundTables,
};
